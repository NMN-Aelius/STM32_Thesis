...
int main(void)
{
	...
	while(1)
	{
		...
		//		{
		//			flag_timer1 = 0;
		uint8_t theta1[2] = {200,20};
		uint8_t theta2[2] = {200,20};
		uint8_t theta3[2] = {200,20};
		uint8_t	theta4[2] = {200,20};
		uint8_t	theta5[2] = {200,20};
		uint8_t	theta6[2] = {200,20};
		TX_CAN_HEADER.StdId = 0x000; // Node 1
		...
		TX_CAN_HEADER.StdId = 0x002; // Node 1
		...
		TX_CAN_HEADER.StdId = 0x004; // Node 1
		...
		TX_CAN_HEADER.StdId = 0x008; // Node 1
		...
		TX_CAN_HEADER.StdId = 0x010; // Node 1
		if(HAL_CAN_AddTxMessage(&hcan1, &TX_CAN_HEADER, theta5, &TxMailBox) == HAL_OK)
		{
			//				HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_13);
		}
		//			delay_us(300);
		...
		if(HAL_CAN_AddTxMessage(&hcan1, &TX_CAN_HEADER, theta6, &TxMailBox) == HAL_OK)
		{
			//				HAL_GPIO_TogglePin(GPIOD, GPIO_PIN_13);
		}
		delay_us(300);
